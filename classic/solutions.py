...
def q2_2():
    ...
    policy, state_value_function, rewards, _ = monte_carlo(
        WORLD, DISCOUNT, n_episodes, epsilon, learning_rate
    )
...
def q2_2_3():
    ...
    for j, learning_rate in enumerate(learning_rates, 5):
        ...
        mean_rewards = np.mean(total_rewards, axis=0)
        plt.plot(
            mean_rewards, label=r"$\alpha$ = " + learning_rate_labels[learning_rate]
        )
    plt.legend(loc="lower right")
...
def q2_3():
    ...
    policy, state_value_function, rewards, _ = q_learning(
        WORLD, DISCOUNT, n_episodes, epsilon, learning_rate
    )
...
def q2_4():
    ...
    for i in range(n_runs):
        print(f"Running {i}")
        _, _, _, q_policies = q_learning(
            WORLD, DISCOUNT, n_episodes, epsilon, learning_rate
        )
        _, _, _, mc_policies = monte_carlo(
            WORLD, DISCOUNT, n_episodes, epsilon, learning_rate
        )
...
def q2_5_1():
    ...
    _, _, mc_rewards, mc_policies = monte_carlo(
        WORLD, DISCOUNT, n_episodes, epsilon, learning_rate
    )
    for j in range(n_episodes):
        mc_policy = mc_policies[j]
...
def q2_5_2():
    ...
    _, _, q_rewards, q_policies = q_learning(
        WORLD, DISCOUNT, n_episodes, epsilon, learning_rate
    )
    for j in range(n_episodes):
        q_policy = q_policies[j]
